...
[Script]
nf_check = type=generic, timeout=30, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/claude_check_no_warp.js, argument=title=Claude+解锁检测
...
[Script]
nf_check = type=generic, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/claude_check_no_warp.js, argument=title=Claude+解锁检测, debug=true
*/
...
(async () => {
  const { region, status } = await testClaude();
  console.log(
    `[${SCRIPT_NAME}] testClaude: region=${region}, status=${status}`,
  );
