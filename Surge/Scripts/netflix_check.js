...
https://raw.githubusercontent.com/dler-io/Rules/refs/heads/main/Surge/Surge%204/Script/netflix_check.js
...
fullStyle: 完整解锁时展示的样式, 参数可选值有 good, info, alert, error. 注意相应的 Icon / Color 会覆盖此设置

onlyOriginalContent：仅解锁自制剧时展示的文本内容

onlyOriginalIcon: 仅解锁自制剧时展示的图标

onlyOriginalIconColor: 仅解锁自制剧时展示的图标颜色

onlyOriginalStyle: 仅解锁自制剧时展示的样式, 参数可选值有 good, info, alert, error. 注意相应的 Icon / Color 会覆盖此设置
...
[Script]
nf_check = type=generic, timeout=30, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/netflix_check.js, argument=title=Netflix+解锁检测
...
[Script]
nf_check = type=generic, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/netflix_check.js, argument=title=Netflix+解锁检测, debug=true
*/
...
const SCRIPT_NAME = "netflix_check";
...
const ERROR_NOT_FOUND = "probe_not_found";
...
(async () => {
  const { region, status } = await testNetflix();
  console.log(
    `[${SCRIPT_NAME}] testNetflix: region=${region}, status=${status}`,
  );
...
/**
 * Run Netflix probe and map result to status.
 *
...
async function testNetflix() {
  try {
    const region = await withTimeout(
      probeNetflixTitle(FILM_ID),
      options.timeout,
    );
    return { region, status: STATUS_AVAILABLE };
  } catch (error) {
    console.log(`[${SCRIPT_NAME}] testNetflix: stage=primary, error=${error}`);
    if (error === ERROR_NOT_FOUND) {
      try {
        const region = await withTimeout(
          probeNetflixTitle(AREA_TEST_FILM_ID),
          options.timeout,
        );
        return { region, status: STATUS_LIMITED };
      } catch (fallbackError) {
        console.log(
          `[${SCRIPT_NAME}] testNetflix: stage=fallback, error=${fallbackError}`,
        );
        if (fallbackError === ERROR_NOT_AVAILABLE) {
          return { status: STATUS_NOT_AVAILABLE };
        }
        if (fallbackError === ERROR_TIMEOUT) {
...
/**
 * Probe Netflix availability using a specific title id.
 *
 * @param {string | number} filmId
 * @returns {Promise<string>} Two-letter region code.
 */
function probeNetflixTitle(filmId) {
  return new Promise((resolve, reject) => {
    ...
    $httpClient.get(requestOptions, (error, response, data) => {
      if (error != null) {
        console.log(
          `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, requestError=${error}`,
        );
        reject(error || ERROR_GENERIC);
        return;
      }

      if (response.status === 403) {
        console.log(
          `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, status=403, reason=not_available`,
        );
      ...
      if (response.status === 404) {
        console.log(
          `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, status=404, reason=not_found`,
        );
      ...
      if (response.status === 200) {
        let region = "us";
        const originatingUrl = response.headers["x-originating-url"];
        if (!originatingUrl) {
          console.log(
            `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, missing=x-originating-url`,
          );
        } else {
          const regionSegment = originatingUrl.split("/")[3] || "";
          const normalizedRegion = regionSegment.split("-")[0];
          if (!regionSegment) {
            console.log(
              `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, missing=region_segment, fallback=us, originatingUrl=${originatingUrl}`,
            );
          } else if (!normalizedRegion) {
            console.log(
              `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, invalid=normalized_region_empty, fallback=us, regionSegment=${regionSegment}`,
            );
          } else if (normalizedRegion === "title") {
            console.log(
              `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, fallback=us, reason=title_segment, regionSegment=${regionSegment}`,
            );
          } else {
      ...
      console.log(
        `[${SCRIPT_NAME}] probeNetflixTitle: filmId=${filmId}, unexpectedStatus=${response.status}`,
      );
...
function setPanelByStatus(targetPanel, parsedOptions, status, region) {
  switch (status) {
    case STATUS_AVAILABLE:
      applyIconOrStyle(
        targetPanel,
        parsedOptions.fullIcon,
        parsedOptions.fullIconColor,
        parsedOptions.fullStyle,
      );
      targetPanel["content"] = replaceRegionPlaceholder(
        parsedOptions.fullContent,
        region,
      );
      return;
    case STATUS_LIMITED:
      applyIconOrStyle(
        targetPanel,
        parsedOptions.onlyOriginalIcon,
        parsedOptions.onlyOriginalIconColor,
        parsedOptions.onlyOriginalStyle,
      );

      if (
        !parsedOptions.onlyOriginalContent.includes("#REGION_FLAG#") &&
        !parsedOptions.onlyOriginalContent.includes("#REGION_CODE#") &&
        !parsedOptions.onlyOriginalContent.includes("#REGION_NAME#") &&
        !parsedOptions.onlyOriginalContent.includes("#REGION_NAME_EN#")
      ) {
        targetPanel["content"] = parsedOptions.onlyOriginalContent;
        return;
      }

      targetPanel["content"] = replaceRegionPlaceholder(
        parsedOptions.onlyOriginalContent,
        region,
      );
      return;
