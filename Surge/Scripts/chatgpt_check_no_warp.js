/*
https://raw.githubusercontent.com/getsomecat/GetSomeCats/refs/heads/Surge/modules/Panel/CFGPT/CFGPT_2_nowarp.js
https://raw.githubusercontent.com/dler-io/Rules/refs/heads/main/Surge/Surge%204/Script/chatgpt_check.js
...
[Panel]
nf_check = script-name=nf_check, title="ChatGPT 解锁检测", content="请刷新", update-interval=1

[Script]
nf_check = type=generic, timeout=30, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/chatgpt_check_no_warp.js, argument=title=ChatGPT+解锁检测
...
[Script]
nf_check = type=generic, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/chatgpt_check_no_warp.js, argument=title=ChatGPT+解锁检测, debug=true
*/

const TRACE_URL = "https://chat.openai.com/cdn-cgi/trace";
const SCRIPT_NAME = "chatgpt_check";
...
const DEFAULT_OPTIONS = {
  ...
  availableIconColor: "",
  availableStyle: "good",
  notAvailableContent: "不支持 | 地区: #REGION_FLAG# #REGION_NAME#",
  notAvailableIcon: "",
...
const SUPPORTED_REGIONS = {
  ...
  AX: { chinese: "奥兰群岛", english: "Aland Islands" },
  ...
  CD: { chinese: "刚果（金）", english: "Congo (DRC)" },
  CR: { chinese: "哥斯达黎加", english: "Costa Rica" },
  CI: { chinese: "科特迪瓦", english: "Côte d'Ivoire" },
  ...
  RE: { chinese: "留尼汪", english: "Réunion" },
  RO: { chinese: "罗马尼亚", english: "Romania" },
  RW: { chinese: "卢旺达", english: "Rwanda" },
  BL: { chinese: "圣巴泰勒米", english: "Saint Barthelemy" },
  SH: { chinese: "圣赫勒拿", english: "Saint Helena" },
  ...
  PM: { chinese: "圣皮埃尔和密克隆", english: "Saint Pierre and Miquelon" },
  VC: {
    chinese: "圣文森特和格林纳丁斯",
...
(async () => {
  const { region, status } = await testChatGPT();
  console.log(
    `[${SCRIPT_NAME}] testChatGPT: region=${region}, status=${status}`,
  );
...
/**
 * Run ChatGPT probe and map result to status.
 *
...
async function testChatGPT() {
  try {
    ...
    return { region: normalizedCountryCode, status };
  } catch (error) {
    console.log(`[${SCRIPT_NAME}] testChatGPT: error=${error}`);
    if (error === ERROR_TIMEOUT) {
      return { region: "", status: STATUS_TIMEOUT };
    }
...
/**
 * Fetch Cloudflare trace metadata from ChatGPT endpoint.
 *
