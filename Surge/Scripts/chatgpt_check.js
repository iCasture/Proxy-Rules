/*
https://raw.githubusercontent.com/getsomecat/GetSomeCats/refs/heads/Surge/modules/Panel/CFGPT/CFGPT_2.js
...
[Script]
nf_check = type=generic, timeout=30, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/chatgpt_check.js, argument=title=ChatGPT+解锁检测
...
[Script]
nf_check = type=generic, script-path=https://raw.githubusercontent.com/iCasture/Proxy-Rules/refs/heads/master/Surge/Scripts/chatgpt_check.js, argument=title=ChatGPT+解锁检测, debug=true
*/
...
(async () => {
  const { region, status, warpState: rawWarpState } = await testChatGPT();
  const warpStatus = mapWarpStateToStatus(rawWarpState);
  console.log(
    `[${SCRIPT_NAME}] testChatGPT: region=${region}, status=${status}, rawWarpState=${rawWarpState}, warpStatus=${warpStatus}`,
  );
